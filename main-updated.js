...
// معالجات قاعدة البيانات والطابعة
function setupDatabaseHandlers() {
    ...
    // الحصول على قائمة الطابعات المتصلة
    ipcMain.handle('get-printers', async (event) => {
        try {
            if (!mainWindow || !mainWindow.webContents) {
                console.error('❌ النافذة الرئيسية غير متوفرة');
                return { success: false, error: 'النافذة غير متوفرة', printers: [] };
            }

            const printers = mainWindow.webContents.getPrinters();
            console.log('✅ تم الحصول على قائمة الطابعات:', printers.length);
    ...
    ipcMain.handle('print-silent', async (event, { html, printerName, options = {} }) => {
        try {
            ...
            // تنفيذ الطباعة
            const success = await printWindow.webContents.print(printOptions, (success, errorType) => {
                if (!success) {
                    console.error('❌ خطأ في الطباعة:', errorType);
                }
            });
